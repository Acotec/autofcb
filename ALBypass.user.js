...
(function () {
            ...
            if (!message) { //if api return with a result
                ...
                if (parseInt(tryagain) <= retry) {
                    ...
                    if (new RegExp(check, 'ig').test(message)) {
                        ...
                        let after24h = new Date(new Date().getTime() + (24 * 60 * 60 * 1000)).getTime();
    ...
    GM_getValue("after24h") == new Date && (GM_setValue("after24h", ""), GM_setValue("Bypass", !0), GM_setValue("already_sent", !1));
